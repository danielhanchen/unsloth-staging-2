...
"""Native-Windows proof for unslothai/unsloth#7897. Skipped everywhere else.

The Linux suite simulates Windows by injecting ntpath. This file asserts the same
properties with the REAL os.path on a real NTFS volume, so the simulation cannot
quietly diverge from the platform it models. GitHub's windows-latest runners
provide a second drive letter (D:), which is what makes the cross-drive case real
