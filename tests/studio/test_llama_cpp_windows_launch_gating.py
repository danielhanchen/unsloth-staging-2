...
mitigations to ``LlamaCppBackend.load_model``:
  ...
  * ``--threads 2`` (correctly gated on ``_fully_gpu_offloaded``)
...
tests lock in the ``_fully_gpu_offloaded`` gate on all three blocks.
...
def _find_threads_gate() -> ast.If:
    """``if _sys.platform == 'win32' and _fully_gpu_offloaded:`` for --threads."""
    ...
    for node in ast.walk(_TREE):
        if (
            ...
            and _has_name(node.test, "_fully_gpu_offloaded")
            and _has_name(node, "_t")
            and not any(
...
def test_threads_gate_requires_full_offload():
    ...
    keeps gating on _fully_gpu_offloaded after future refactors."""

    node = _find_threads_gate()
    assert _has_name(node.test, "_fully_gpu_offloaded"), (
        "--threads 2 gate must keep _fully_gpu_offloaded in its condition."
    )
...
def test_cache_flags_block_is_gated_on_full_offload():
    ...
    assert _has_name(node.test, "_fully_gpu_offloaded"), (
        "Cache-flags block must gate on _fully_gpu_offloaded; otherwise "
        "Windows CPU-only / partial-offload runs lose prompt-cache reuse "
        "across turns. Found condition: "
        f"{test_src}"
    )
...
def test_omp_env_block_is_gated_on_full_offload():
    ...
    assert _has_name(node.test, "_fully_gpu_offloaded"), (
        "OMP env block must gate on _fully_gpu_offloaded; got: "
...
def test_cache_flags_block_behavior(platform, fully_offloaded, expect_cache_flags):
    ...
    cmd: list[str] = []
    ns = {"_sys": _sys, "_fully_gpu_offloaded": fully_offloaded, "cmd": cmd}
...
def test_omp_env_block_behavior(fully_offloaded, expect_omp):
    ...
    env: dict = {}
    ns = {"_fully_gpu_offloaded": fully_offloaded, "env": env}
...
def test_threads_gate_behavior(platform, fully_offloaded, expect_threads_2):
    ...
    if expect_threads_2:
        ...
        assert ns["_t"] != 2, (
            ...
            f"fully_offloaded={fully_offloaded}: _t={ns['_t']!r}"
        )
