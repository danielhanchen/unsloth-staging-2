...
A model too large for its GPU gets partially offloaded, and accelerate leaves
...
and the user is shown one of

    TypeError: 'NoneType' object is not subscriptable
    NotImplementedError: Cannot copy out of meta tensor; no data!

Neither names the model, the offload, or what to do about it. All three of
`Qwen3_MoE`, `Qwen3_5_MoE` and `Nemotron-3-Nano-30B` failed to export this
way, and every one had already logged accelerate's "Some parameters are on
the meta device because they were offloaded to the cpu" much earlier, where
nobody was looking.

This adds a sentence to the existing error. The risk it has to avoid is
mislabelling: appending an offload explanation to a failure that had nothing
to do with offloading would send someone hunting the wrong thing, so the hint
is empty unless a meta parameter is actually present.
"""
