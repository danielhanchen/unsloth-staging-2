...
does. Running with UNSLOTH_COMPILE_DISABLE=1, the documented escape hatch for a
compile problem, removes the generated module and the guard with it while trl's
write stays. That is how the Sudoku isolation run traded a TorchRuntimeError for
this AttributeError. The wrapper in `trainer.py` runs in both modes, so the
guard belongs there as well.
