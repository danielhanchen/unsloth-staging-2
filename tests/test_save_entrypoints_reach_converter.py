...
This exists because of a real regression, and the way it hid is the point.

Adding the `_gguf_failure_looks_like_disk` helper put its definition in the
*middle* of `unsloth_save_pretrained_gguf` rather than after it. Python was
perfectly happy: the first half of the function still parsed, still ran, still
did the 16-bit merge, and still returned -- it just returned before ever
calling `save_to_gguf`. Everything after the insertion point became dead code
stranded inside the helper, below its `return False`.

So `save_pretrained_gguf(...)` raised nothing, printed its usual progress, left
a populated merge directory behind, and produced no `.gguf` at all. From the
caller's side that is indistinguishable from success until you go looking for
the file. It cost a full 12-notebook sweep before anyone noticed.

Nothing that reads like a normal unit test would have caught it -- the halves
are individually valid, and importing `unsloth.save` still worked. The
structural properties below are what actually fail when a function gets cut in
two, so they are checked directly against the AST. No unsloth import, so this
stays fast and runs without a GPU or any of the heavy deps.
"""
