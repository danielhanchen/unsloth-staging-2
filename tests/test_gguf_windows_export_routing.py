...
"""End-to-end routing proof for unslothai/unsloth#7897, run on Linux.

save.py's module-global ``os`` is swapped for a shim whose *pure* path functions
come from ``ntpath``, so ``save_to_gguf`` performs the exact join arithmetic a
Windows host would. The assertion is on the ``output_gguf`` argument that reaches
``quantize_gguf`` (save.py:2073), which is where the GGUF escaped the export
directory.

Requires torch + unsloth_zoo (the temp/venvs/saving sandbox).
"""
