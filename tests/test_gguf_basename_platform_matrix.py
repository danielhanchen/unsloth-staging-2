...
Honest framing: the fix is pure path arithmetic and imports no GPU library, so the
GPU axis is an INVARIANCE check, not a behavioural one. This file asserts exactly
that -- the derived GGUF stem and its destination are byte-identical in every cell
-- rather than inventing per-vendor expectations that do not exist.

The GPU cell is selected by UNSLOTH_SIM_GPU (nvidia|rocm|cpu) and applied at import
time, because the spoofs mutate torch/hardware module globals and cannot be undone
in-process (tests/_zoo_aggressive_cuda_spoof.py guards on
torch.cuda._unsloth_consolidated_spoof). Run one process per cell; the driver is
tests/run_platform_matrix.sh.

The OS axis needs no process isolation: it is monkeypatch-scoped path flavour.
"""
