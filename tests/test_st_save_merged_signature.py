...
FastLanguageModel exposes

    save_pretrained_merged(save_directory, tokenizer = None,
                           save_method = "merged_16bit", ...)

and every notebook and doc calls it positionally:

    model.save_pretrained_merged("dir", tokenizer, save_method="merged_16bit")

FastSentenceTransformer bound the same public name to
`(self, save_directory, **kwargs)`, so that exact call answered with

    TypeError: _save_pretrained_merged() takes 2 positional arguments but
    4 were given

One method name, two incompatible signatures. Five embedding notebooks hit
it in the sweep. Keyword callers were unaffected, which is why it survived.

`save_method` is not merely accepted and dropped. The first definition
merges and unloads unconditionally, so ignoring `save_method="lora"` there
would write full weights for a request to write adapters -- a wrong answer
is worse than the TypeError. It refuses instead. The second definition
forwards to the FastLanguageModel merge, which understands every method, so
