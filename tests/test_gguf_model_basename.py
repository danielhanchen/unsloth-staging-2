...
"""GGUF output filenames must never carry a directory, drive or UNC prefix.

Regression cover for unslothai/unsloth#7897: when a LoRA's base model is a local
*Windows* directory, ``config._name_or_path`` has no forward slash, so the old
``base_model_name.split("/")[-1]`` kept the whole path as the "model name". Every
GGUF filename was then built by joining that path onto the output directory --
and ``ntpath.join`` DISCARDS its first argument when the second carries a drive:

    ntpath.join(r"C:\\exp\\_gguf", r"D:\\M\\X.Q5_K_M.gguf") == r"D:\\M\\X.Q5_K_M.gguf"

so the finished GGUF landed next to the *base model* instead of the export
directory, and Studio reported ``Final files in <save_dir>: (none)``.

These tests run on Linux and still assert the Windows answers -- that is
deliberate. ``os.path.basename`` would return the whole ``D:\\...`` string on a
POSIX host, so a fix built on it would pass here while Windows stayed broken.
"""
