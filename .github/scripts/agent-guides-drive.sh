...
#
# Connection probe: tell the model to reply directly and DISABLE the actionable
# tools. A capable model otherwise treats "reply pong" as a task and emits a
# tool_use, which -p mode relays as no visible text (an empty transcript).
CLAUDE_CONNECT_FLAGS=(
  --system-prompt-file "$SCRIPT_DIR/ci-connect-prompt.txt"
  --disallowedTools Bash Edit Write Read Glob Grep WebFetch WebSearch Task NotebookEdit Agent
)
# File-edit: the task needs the file/shell tools, so allow just those.
CLAUDE_EDIT_FLAGS=(
