...
# and restricting to the handful of tools the smoke test needs (--allowedTools)
# cuts the prefill to a few hundred tokens so it completes quickly on CPU. These
# only shape the request size; the connect.py recipe (endpoint, auth, model) is
...
case "$MODE" in
  ...
  connection)
    ...
    if [ "$AGENT" = "pi" ]; then
      ...
      case "$AGENT" in
        claude)   invoke_via_connect "$OUT" "${CLAUDE_MIN_FLAGS[@]}" -p "$PROMPT" ;;
        ...
        hermes)   invoke_via_connect "$OUT" --ignore-rules -z "$PROMPT" ;;
        openclaw) invoke_via_connect "$OUT" agent --local --agent main \
      ...
      case "$AGENT" in
        ...
        claude)
          ...
          if [ "$cont" = "continue" ]; then
            invoke_via_connect "$out" "${CLAUDE_MIN_FLAGS[@]}" --dangerously-skip-permissions -p --continue "$prompt"
          else
            invoke_via_connect "$out" "${CLAUDE_MIN_FLAGS[@]}" --dangerously-skip-permissions -p "$prompt"
          fi ;;
        codex)
