...
# machine has the runtime. A container CAN answer it. Record the truth either way;
# do not fail on it here -- the install job is where it becomes a verdict.
foreach ($dll in 'vcruntime140.dll', 'vcruntime140_1.dll', 'msvcp140.dll') {
    $p = Join-Path $env:WINDIR "System32\$dll"
    Write-Host ("  {0,-20} {1}" -f $dll, $(if (Test-Path $p) { 'PRESENT' } else { 'ABSENT' }))
}
