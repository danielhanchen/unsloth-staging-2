...
async def open_chat(
    ...
    """Launch a headless browser (`engine`, chromium by default), seed init
    scripts, open `/chat`.
    ...
    try:
        async with async_playwright() as p:
            # `engine` defaults to chromium so every existing caller is
            # unchanged. It exists because some questions are ENGINE-SPECIFIC
            # and Chromium is the wrong venue for them: `content-visibility:
            # auto` is unlocked for print by Chromium and resolved to
            # `visible` by Gecko, while WebKit does neither, and WebKit is
            # what Studio and Desktop render through on Linux. A scene that
            # only ever ran on Chromium cannot speak to that.
            if engine not in ("chromium", "webkit", "firefox"):
                raise ValueError(f"unknown engine {engine!r}")
            launcher = getattr(p, engine)
            browser = await launcher.launch(headless=headless, slow_mo=slow_mo_ms)
