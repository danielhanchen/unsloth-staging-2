...
async def dismiss_modals(sp: StudioPage, timeout_ms: int = 2000) -> list[str]:
    """Close anything overlaying the chat, returning what it closed.

    A Studio that finds a newer llama.cpp opens an update dialog on first
    paint. It sits over the composer, so every click below it fails with a
    timeout that reads like a missing element rather than an intercepted one.
    """
    closed = []
    for label in ("Remind me later", "Later", "Not now", "Dismiss", "Close"):
        loc = sp.page.locator(f'button:has-text("{label}")')
        try:
            if await loc.count() and await loc.first.is_visible():
                await loc.first.click(timeout=timeout_ms)
                closed.append(label)
                await sp.page.wait_for_timeout(400)
        except Exception:
            continue
    return closed
...
async def pick_model(sp: StudioPage, model_id: str, timeout_ms: int = 15000) -> None:
    ...
    Exact match matters: a substring selector for "gemini-2.5-flash" also
    takes "gemini-2.5-flash-image". The comparison happens in Python rather
    than in a selector, because a pattern handed to Playwright is serialised
    into its own selector grammar, and the `/` in every HF model id parses as
    a delimiter there.
    """
    page = sp.page
    await dismiss_modals(sp)
    # The trigger is not scoped to the composer: current builds render it above
    # the form and carry no testid, so a form-scoped testid selector finds
    # nothing and times out. Try the specific selectors first, then the label.
    display = model_id.strip().rsplit("/", 1)[-1]
    # The trigger is the only button that opens a dialog, and it reads
    # "Select model" when empty or the loaded model's name afterwards, so match
    # on the popup role rather than on either label.
    trigger = page.locator(
        '[data-testid="model-picker-trigger"], '
        'button[aria-haspopup="dialog"], '
        'button:has-text("Select model")'
    ).first
    # Studio remembers the last model across sessions, so re-selecting the one
    # already loaded is a no-op rather than an error.
    try:
        current = (await trigger.inner_text()).strip().splitlines()[0].strip()
    except Exception:
        current = ""
    if current in (model_id.strip(), display):
        return
    await trigger.click(timeout=timeout_ms)

    # Match in Python, not in the selector. A regex handed to `get_by_role` is
    # serialised into Playwright's own selector grammar, and every HF model id
    # contains a `/`, which that grammar reads as a delimiter:
    #   InvalidSelectorError: ... unexpected symbol "e" at position 26
    # So the anchored-regex form could never select an org-prefixed model.
    wanted = model_id.strip()
    options = page.get_by_role("option")
    if not await options.count():
        # Newer builds open a search dialog instead of a listbox: tabs across
        # the top, results as buttons. Type the id so the list narrows to it
        # rather than paging through several hundred models.
        dialog = page.locator('[role="dialog"]').first
        await dialog.wait_for(state="visible", timeout=timeout_ms)
        box = dialog.locator('input[placeholder], input[type="search"]').first
        if await box.count():
            await box.fill(wanted)
            await page.wait_for_timeout(1200)
        options = dialog.get_by_role("button")
    await options.first.wait_for(state="visible", timeout=timeout_ms)
    # The picker labels a model by its display name, so `unsloth/x` shows as
    # `x`. Accept either, full id first so an exact id always wins.
    accepted = {wanted, wanted.rsplit("/", 1)[-1]}
    deadline = time.monotonic() + timeout_ms / 1000
    while True:
        for pass_wanted in (wanted, None):        # exact id, then display name
            for i in range(await options.count()):
                ...
                try:
                    ...
                    continue
                # Exact match on purpose: "gemini-2.5-flash" must not take
                # "gemini-2.5-flash-image". Compare the first line, since an
                # entry can carry a size or badge underneath the name.
                head = label.splitlines()[0].strip() if label else ""
                hit = {head, label}
                if (pass_wanted in hit) if pass_wanted else bool(hit & accepted):
                    await opt.click(timeout=timeout_ms)
                    return
        if time.monotonic() >= deadline:
            seen = [(await options.nth(i).inner_text()).splitlines()[0].strip()
                    for i in range(min(await options.count(), 10))]
            raise AssertionError(f"model {model_id!r} not in the picker; saw {seen}")
