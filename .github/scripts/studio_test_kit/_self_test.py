...
def test_pick_model_uses_exact_match() -> None:
    """A6: pick_model must select the exact model, driven for real.

    Two regressions this pins. A pattern handed to Playwright is serialised
    into its own selector grammar, where the `/` in every HF model id parses
    as a delimiter, so the old anchored-regex form could not select an
    org-prefixed model at all. And an exact match must not be a prefix match:
    "gemma-3-270m-it-GGUF" must not take "gemma-3-270m-it-GGUF-extra".
    """
    import asyncio
    from studio_test_kit import ui
    ...
    class _El:
        ...
        async def inner_text(self): return self.label
    ...
    class _Set:
        ...
        @property
        def first(self): return self.els[0] if self.els else _El()
        async def count(self): return len(self.els)
        def get_by_role(self, role): return _Set(self.els)
        def locator(self, sel): return _Set([])

    def run(labels, wanted):
        opts = [_El(l) for l in labels]
        class _Page:
            def locator(self, sel):
                # the trigger, then the (absent) dialog
                return _Set([_El("Select model")])
            def get_by_role(self, role): return _Set(opts)
            async def wait_for_timeout(self, ms): return None
        class _SP: page = _Page()
        asyncio.run(ui.pick_model(_SP(), wanted, timeout_ms=1000))
        return opts

    opts = run(["unsloth/gemma-3-270m-it-GGUF-extra",
                "unsloth/gemma-3-270m-it-GGUF"], "unsloth/gemma-3-270m-it-GGUF")
    assert opts[1].clicked and not opts[0].clicked, "took a prefix collision"

    # The picker labels by display name, so the org prefix is absent there.
    opts = run(["gemma-3-270m-it-GGUF-extra", "gemma-3-270m-it-GGUF"],
               "unsloth/gemma-3-270m-it-GGUF")
    assert opts[1].clicked and not opts[0].clicked, "display-name match failed"
    _ok("ui.pick_model exact match, slashes and prefixes")
