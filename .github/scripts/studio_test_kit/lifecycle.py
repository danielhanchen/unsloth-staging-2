...
-p <port>` detached and tails the log for the bootstrap
...
class StudioInstall:
    ...
    # Seconds from launch to the first 200 on /healthz. The poll loop below already
    # measured this and threw it away, so every flow that launches a Studio was
    # sitting on a startup benchmark it never recorded. None when wait_for_healthz
    # is off. For a full per-phase breakdown use unsloth/scripts/profile_startup.py.
    t_healthz_s: Optional[float] = None
...
def _read_password_from_file(home: Path) -> Optional[str]:
    """Read the bootstrap password Studio writes to disk.

    Current Studio does not print the secret inline; it logs "password saved to:
    <home>/auth/.bootstrap_password" and writes it there. Log-scraping alone
    therefore returns None, login is skipped, and /chat renders the login screen
    instead of the composer.
    """
    for candidate in (home / "auth" / ".bootstrap_password",
                      home / ".bootstrap_password"):
        try:
...
def launch_studio(
    ...
    # No `setsid`/`bash -c ... | tee`: setsid does not exist on macOS or Windows, and
    # start_new_session already detaches on POSIX. Writing straight to the log file
    # also removes the BrokenPipeError class the README warns about, since there is
    # no pipe reader that can die while the server lives on.
    popen_kw: dict = {}
    if os.name == "nt":
    ...
    # Clock from the spawn, not from the start of the poll loop. Recovering the
    # password below blocks until Studio prints it, which is already most of the
    # startup, so a poll-relative figure reported ~0.1s for a boot the log shows
    # taking seconds.
    ...
    if wait_for_healthz:
        ...
        healthz_started = time.time()
        healthz_deadline = healthz_started + healthz_timeout_s
        while time.time() < healthz_deadline:
            try:
                with urllib.request.urlopen(url, timeout=2) as r:
                    if r.status == 200:
                        t_healthz_s = round(time.time() - spawned_at, 2)
                        break
            except Exception:
                time.sleep(1)
        else:
    ...
    install.t_healthz_s = t_healthz_s

    # After the health check, not before. Studio writes the password around the
    # time it starts serving, so reading first put a blocking wait (and, when the
    # line never matched, its whole timeout) inside the startup measurement.
    # File first: authoritative, and present even on a relaunch that reprints
    # nothing. Log scraping stays as the fallback for older Studios.
    install.bootstrap_password = (
        _read_password_from_file(install.home)
        or _read_password_from_log(log_path, time.time() + password_timeout_s)
    )

    # pid already captured from Popen above; keep the pgrep fallback for a Studio
    # this process did not spawn.
    try:
        if install.pid is None and os.name != "nt":
...
def stop_studio(install: StudioInstall) -> None:
    """Best-effort stop of the launched Studio and its children."""
    if not install.pid:
        return
    if os.name == "nt":
        # No process groups on Windows; /T walks the tree the way the app's
        # windows_job.rs has to.
        subprocess.run(["taskkill", "/PID", str(install.pid), "/T", "/F"],
                       capture_output=True)
        return
    try:
