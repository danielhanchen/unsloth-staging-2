...
Note (2026-09, macos-26): the default mlx-community/Qwen3.5-2B-4bit fails `grad_norm_bounded`
(loss ~11) when mlx-vlm resolves to 0.6.4, which zoo's Apple Silicon `transformers<=5.5.0` cap
forces (mlx-vlm >= 0.6.5 needs transformers >= 5.14). 0.6.4's qwen3_5 sanitize adds +1.0 to the
already-offset RMSNorm weights; Unsloth's text_only load goes through mlx-vlm, so CE is 12.65 vs
0.816 via mlx-lm. With mlx-vlm 0.6.5 / 0.7.2 (staging_ci --job-deps "transformers mlx-vlm mlx-lm")
CE is 0.8155 and this preset passes (loss 2.08 -> 1.27). jobs/mlx_diag_qwen35.py reproduces it.
"""
...
def _vtuple(v):
    """'0.6.10rc1' -> (0, 6, 10): numeric release parts only, enough for a >= floor check."""
    import re
...
def main(argv=None):
    ...
    with C.JobRecorder(a, backend_hint=None) as rec:
        ...
        if not rec.check("mlx_active", is_mlx, f"unsloth._IS_MLX={is_mlx} on {C.detect_device()}"):
            return
        rec.backend_check()
        # mlx-vlm < 0.6.5 double-offsets Qwen3.5's RMSNorm (see the module note); zoo's transformers
        # cap is what pulls it in. Require the fixed release instead of training a broken model.
        vlm = rec.data["versions"].get("mlx-vlm")
        if not rec.check("mlx_vlm_ge_0_6_5", vlm is None or _vtuple(vlm) >= (0, 6, 5),
                         f"mlx-vlm {vlm}; fix: pip install -U transformers mlx-vlm mlx-lm"):
            return
