"""One-off diagnostic: why does mlx-community/Qwen3.5-2B-4bit score CE ~0.97 through mlx_lm.load
but ~11.1 through mlx_vlm.load (and Unsloth text_only, which routes to mlx-vlm)?

    python jobs/mlx_diag_qwen35.py [--model M] [--out diag.json] [--no-unsloth]

Checks, each isolated (a failure is recorded, never raised): versions + config; parameter
names / shapes / values of the text stack loaded both ways, plus the raw checkpoint norms;
per-layer hidden-state divergence on one token batch; mlx-vlm with explicit text positions;
Unsloth's text_only load. Prints `DIAG {json}` and a summary, always exits 0.
"""
...
def main():
    ...
    ap.add_argument("--tiny", action="store_true", help="accepted for staging_ci; ignored")
    ap.add_argument("--mlx-vlm", default=None, metavar="VERSION",
                    help="pip install this mlx-vlm (with the transformers it needs) first, then "
                         "re-run; run it as the LAST job of a leg, it changes the environment")
    a = ap.parse_args()
    ...
    # 2b. Raw checkpoint norms vs what each loader holds (detects a double +1 RMSNorm offset).
    def _norms():
        raw = {}
    ...
    # 3. Per-layer hidden states on the same batch: record every decoder layer's output.
    def _record(text_model, store):
        layers = text_model.model.layers
        pos = {id(layer): i for i, layer in enumerate(layers)}  # nn.Module is a dict: never ==
        patched = []
