...
#
# This is a RUNTIME redistributable, not the MSVC compiler: the prebuilt llama-server
# and PyTorch link against it, so unlike CMake and VS Build Tools (both now
# detection-only) it genuinely has to be present. Without it torch fails to import
# with a missing VCRUNTIME140.dll, which looks like a broken install rather than a
# missing system package.
#
# winget is not always available (LTSC, Server, managed corporate images), and on
# those hosts this used to silently do nothing while the install still reported
...
function Ensure-VCRedist {
    ...
    if (-not (Test-VCRedistInstalled)) {
        # aka.ms/vs/17/release/vc_redist.<arch>.exe is Microsoft's evergreen link for
        # the current 2015-2022 runtime. /quiet /norestart so it never blocks or
        # reboots an unattended install.
...
# ============================================
# The old comment here said git was "required by pip for git+https:// deps and by
# npm", and a failure to install it was fatal. Neither holds on the consumer path:
# the git+https URL for unsloth-zoo is only used under STUDIO_LOCAL_INSTALL, node is
# a pinned nodejs.org prebuilt that never touches system npm, and the frontend
# lockfile has no VCS dependencies. Making git fatal therefore blocked clean Windows
# boxes with no winget (LTSC, Server, managed corporate images) for a tool the
# install did not use -- the same over-requirement macOS had with the Xcode CLT.
