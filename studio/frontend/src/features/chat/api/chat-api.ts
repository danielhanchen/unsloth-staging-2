...
/**
 * PR-C2: batched messages fetch. Pre-PR-C2 the sidebar / search index
 * rebuild ran `Promise.all(threads.map(t => listChatMessages(t.id)))`,
 * which is N HTTP round-trips for N threads — easily 200+ on long-running
 * installs and a major cause of stream-time sidebar lag. The backend
 * endpoint accepts up to ~10k thread ids per request and returns a map
 * keyed by threadId.
 *
 * Falls back to per-thread `listChatMessages` if the backend returns
 * 404 / 405 (older Studio versions without the batch route).
 */
