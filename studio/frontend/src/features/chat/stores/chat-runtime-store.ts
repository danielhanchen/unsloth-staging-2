...
// PR-C3: settings write debounce + key coalescing.
//
// Pre-PR-C3 every slider drag tick called saveSettingsPatch which appended
// one HTTP write to a serial chain. A 300ms drag = ~18 writes; combined
// with the persistence-storm finding (slider onChange fires on every
// frame), users with slow networks queued unbounded work.
//
// Now: we coalesce into one pendingPatch (last-write-wins per key, deep
// merge for nested inferenceParams), flush on a 400ms trailing edge, AND
// flush synchronously on `beforeunload` so an in-flight pending patch
// isn't lost when the user closes the tab. Boolean toggles still feel
// instant because the in-memory state updates immediately; only the HTTP
// write is debounced.
...
// PR-C3: flush on unload so a pending patch isn't lost if the user closes
// the tab during the 400ms debounce window.
if (typeof window !== "undefined") {
  window.addEventListener("beforeunload", () => {
    if (pendingTimer !== null) clearTimeout(pendingTimer);
    if (Object.keys(pendingPatch).length === 0) return;
    // Synchronous best-effort flush; we don't await because beforeunload
    // doesn't honor it. savePersistedChatSettingsPatch should use sendBeacon
    // or `keepalive: true` fetch under the hood for this to actually land.
...
export const useChatRuntimeStore = create<ChatRuntimeStore>((set, get) => ({
  ...
  hydratePersistedSettings: async () => {
    ...
    settingsHydrationPromise = (async () => {
      ...
      try {
        ...
        // PR-B1: a transient hydrate failure must not permanently disable
        // persistence. Treat the failure as "hydrated with in-memory
        // defaults" so subsequent setParams calls reach saveSettingsPatch.
        // saveSettingsPatch will surface its own toast on real network
        // failures, so the user still gets feedback when persistence is
        // actually broken.
        warnSettingsPersistenceFailure();
  ...
  setParams: (params) =>
    set((state) => {
      // PR-B2: bump mutation versions for changed keys EVEN when not
      // hydrated, so a slow hydration response's version check
      // (getHydratedSettingsState) sees the user's edit and skips the
      // overwrite. Only the saveSettingsPatch call is gated on
      // settingsHydrated; the version bump is unconditional.
