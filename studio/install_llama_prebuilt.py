...
def main() -> int:
    ...
    if args.resolve_prebuilt is not None:
        # Host-aware "is a prebuilt available" probe (no download). Default
        # --published-repo means "pick the repo for this host"; an explicit
        # non-default repo is honored. PrebuiltFallback == source build.
        host = _apply_host_overrides(
            detect_host(),
