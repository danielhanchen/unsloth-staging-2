...
def test_cmd_shellout_is_screened_through_mangled_switches(monkeypatch, command):
    ...
    # _BLOCKED_COMMANDS folds in the Windows names at import time, so patching
    # sys.platform is too late; fake the resolved set instead, the same way the
    # rest of this file fakes the platform.
    monkeypatch.setattr(
        tools,
        "_BLOCKED_COMMANDS",
