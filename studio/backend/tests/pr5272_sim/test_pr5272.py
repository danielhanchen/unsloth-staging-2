...
After PR-A applies (A1 subject scoping, A2 hijack guard, A3 confirm flag,
...
def test_migration_from_pre_pr_db(env):
    ...
    conn.execute(
        ...
        "VALUES ('old-1', 'Pre-PR thread', 'base', 1)"
    )
    conn.commit()
    conn.close()
...
def test_migration_legacy_subject_invisible_to_real_user(env):
    """Pre-PR rows migrated under `__legacy_unscoped__` should NOT appear to
...
def test_two_subjects_isolated_via_http(http_env):
    """Alice creates a thread; Bob must NOT see it via GET /threads. PR-A1."""
    _, _, ch = http_env
    ...
    assert all(t["id"] != "alice-private" for t in threads), (
        "PR-A1 failed: Bob still sees Alice's threads"
    )
...
def test_http_clear_requires_confirm(http_env):
    """PR-A3: DELETE /api/chat without ?confirm=true must 400."""
    client, *_ = http_env
    for i in range(3):
...
def test_http_bulk_replace_rejects_mismatched_threadId(http_env):
    """PR-A2 part 2: replace_thread_messages must reject body messages whose
...
def test_http_upsert_message_rejects_cross_thread_reparent(http_env):
    """PR-A2: PUT /threads/B/messages/m1 when m1 already exists under A → 409."""
    client, *_ = http_env
    client.post("/api/chat/threads", json=_thread("a"))
    client.post("/api/chat/threads", json=_thread("b"))
    # Create m1 under a
