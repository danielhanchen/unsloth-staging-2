...
def test_e2e_defaults_unchanged():
    ...
    cmd, env, pop = _compose_launch(
        base_cmd=BASE, probed_gpus=[(0, 45000), (1, 8000)], gpu_ids=None,
        keep_model_in_vram=False, mlock=False, is_rocm=False, is_vulkan=False,
    )
    assert cmd == BASE
    assert env == {}
    assert pop == []
...
def test_e2e_keep_resident_nvidia_single_gpu():
    cmd, env, pop = _compose_launch(
        base_cmd=BASE, probed_gpus=[(0, 45000), (1, 8000)], gpu_ids=[0],
        keep_model_in_vram=True, mlock=False, is_rocm=False, is_vulkan=False,
    )
...
def test_e2e_amd_rocm_second_gpu_only_keep_resident_and_mlock():
    ...
    cmd, env, pop = _compose_launch(
        base_cmd=BASE, probed_gpus=[(0, 45000), (1, 8000)], gpu_ids=[1],
        keep_model_in_vram=True, mlock=True, is_rocm=True, is_vulkan=False,
    )
...
def test_e2e_amd_rocm_both_gpus():
    cmd, env, pop = _compose_launch(
        base_cmd=BASE, probed_gpus=[(0, 45000), (1, 8000)], gpu_ids=[0, 1],
        keep_model_in_vram=False, mlock=False, is_rocm=True, is_vulkan=False,
    )
...
def test_e2e_vulkan_pin_via_device_arg():
    cmd, env, pop = _compose_launch(
        base_cmd=BASE, probed_gpus=[(0, 45000), (1, 8000)], gpu_ids=[1],
        keep_model_in_vram=True, mlock=False, is_rocm=False, is_vulkan=True,
    )
...
class _FakeBackend:
    ...
    def __init__(self, *, is_loaded=True, keep_resident=False):
