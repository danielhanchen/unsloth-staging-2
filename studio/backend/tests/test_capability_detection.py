...
class TestRawConfigVisionReader:
    @pytest.mark.parametrize(
        ...
        [
            ({"model_type": "deepseek_vl_v2",
              ...
              "vision_config": {}, "projector_config": {}}, True),
    ...
    def test_reader(self, tmp_path, payload, expected):
        ...
        with patch("utils.models.model_config.is_local_path", return_value=False), \
...
def test_gpu_estimate_probe_is_code_free():
    ...
    cfg = {"model_type": "glm4_moe_lite", "hidden_size": 4096,
           "num_hidden_layers": 40, "max_position_embeddings": 8192}
    with patch("utils.transformers_version._load_config_json", return_value=cfg), \
         patch("transformers.AutoConfig.from_pretrained",
               side_effect=AssertionError("AutoConfig must not be called")):
...
def test_models_route_source_has_no_remote_code_probe():
    # The model-details max_position_embeddings fallback must no longer build a
    # trust_remote_code AutoConfig probe.
    import inspect
