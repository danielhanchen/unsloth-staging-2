...
def _seed_message(conn, thread = "t1", message = "m1"):
    conn.execute(
...
def test_committed_rows_survive_a_reopen(db):
    ...
    try:
        assert conn.execute(
            "SELECT id FROM chat_messages WHERE id = 'm-durable'"
...
@pytest.mark.parametrize(
    "sql, dirties, why",
    [
        ("UPDATE chat_messages SET metadata_json='{\"a\":1}' WHERE id='m1'", False,
         "a generation status change touches no attachment"),
        ("UPDATE chat_messages SET role='user' WHERE id='m1'", False,
         "role is not an inventory input"),
        ("UPDATE chat_messages SET attachments_json='[{}]' WHERE id='m1'", True,
         "attachments changed"),
        ("UPDATE chat_messages SET content_json='[{\"t\":1}]' WHERE id='m1'", True,
...
def test_upgrade_replaces_the_unscoped_trigger(tmp_path, monkeypatch):
    ...
    try:
        ...
        conn.commit()
        assert "UPDATE OF" not in conn.execute(
    ...
    conn = studio_db.get_connection()
    try:
        assert "UPDATE OF attachments_json, content_json" in conn.execute(
            "SELECT sql FROM sqlite_master WHERE name='chat_attachment_inventory_dirty_update'"
        ).fetchone()[0]
        _seed_message(conn)
        studio_db._mark_chat_attachment_inventory_clean(conn)
...
def _make_run(run_id = "r1", thread = "t1", status = "queued"):
    studio_db.upsert_chat_thread(
        {"id": thread, "title": "T", "modelType": "gguf", "createdAt": 1}
    )
    studio_db.upsert_chat_message(
        {"id": f"u-{run_id}", "threadId": thread, "role": "user", "content": [],
         "createdAt": 2}
    )
    studio_db.upsert_chat_message(
        {"id": f"a-{run_id}", "threadId": thread, "parentId": f"u-{run_id}",
         "role": "assistant", "content": [], "createdAt": 3}
    )
    research_runs_db.create_run(
        run_id = run_id, owner_subject = "sub", thread_id = thread,
        user_message_id = f"u-{run_id}", assistant_message_id = f"a-{run_id}", config = {},
    )
    if status != "queued":
        conn = studio_db.get_connection()
        try:
...
def test_a_real_sqlite_fault_does_not_stop_the_supervisor(caplog):
    ...
    ladder = _Ladder(
        [sqlite3.OperationalError("no such table: research_runs"), None, None], logger
    )
