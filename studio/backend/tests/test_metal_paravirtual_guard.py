...
"fix" the corrupt output by throwing away Metal acceleration for every real user, so
these tests pin the discrimination, not just the fallback.

Evidence for the guard: on macos-14 and macos-15, which expose "Apple Silicon (Apple
Paravirtual device)", the same model, quant and binary produce
`&#!56789:;<=>@ABCDEFG...` with GPU offload and a correct English sentence with
gpu_layers=0. MLX on the same machine is coherent, which exonerates everything except
the Metal offload path.
"""
...
def _fake_mlx(device_name: str) -> types.ModuleType:
...
def test_only_virtualised_apple_gpus_fall_back(monkeypatch, device_name, expected):
    ...
    # A real Mac must reach the verdict without shelling out at all when MLX already
    # answered "paravirtual"; when it did not, system_profiler is consulted. Return
    # something inert so the test measures the name matching, not the probe.
    monkeypatch.setattr(
...
def test_non_darwin_never_pays_for_the_probe(monkeypatch):
    """Linux and Windows must short-circuit: there is no Metal to be virtualised."""
...
def test_system_profiler_catches_it_when_mlx_is_absent(monkeypatch):
    """MLX is not installed on every Mac. Without this fallback a virtualised machine
    with no MLX would be treated as bare metal and would emit noise."""
    monkeypatch.setattr(sys, "platform", "darwin")
...
def test_a_broken_probe_leaves_gpu_offload_alone(monkeypatch):
    """If neither MLX nor system_profiler can answer, assume a real Mac. Guessing
    'virtualised' would silently halve inference speed for everyone the probe fails on."""
    monkeypatch.setattr(sys, "platform", "darwin")
