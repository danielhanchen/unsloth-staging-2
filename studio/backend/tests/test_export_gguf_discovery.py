...
"""GGUF export must find its output, or fail honestly (unslothai/unsloth#7897).

`save_pretrained_gguf` already returns the files it wrote, but export_gguf used to
discard the return value and guess: cwd, new subdirs of the save dir, and a
`<checkpoint>_gguf` directory. A GGUF written anywhere else -- e.g. next to the
base model, which is what a Windows local base-model path caused -- was invisible,
and the export still reported success over an empty directory.
