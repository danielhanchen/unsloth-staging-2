...
Studio's Chat tab keeps several conversations streaming at once, so a model
load/unload has to know which chats it would interrupt instead of silently
tearing llama-server out from under them.

Everything under test is a plain dict + threading.Lock, so these pass unchanged
...
def test_overlapping_runs_on_one_thread_both_register():
    # A tool continuation can register its next leg before the previous one
    # unwinds; a thread_id-keyed dict would silently drop one of them.
...
def test_cancel_thread_leaves_siblings_alone():
    # The whole point of per-thread Stop: one conversation stops, the rest keep
    # generating and llama-server is never signalled.
    a, b = threading.Event(), threading.Event()
    with active_generations.ActiveGeneration(a, thread_id = "t1"):
        with active_generations.ActiveGeneration(b, thread_id = "t2"):
...
def test_cancel_does_not_unregister_entries():
    # Entries are removed by their own __exit__ so a generation caught mid
    # cleanup is neither double-counted nor lost.
    a = threading.Event()
    with active_generations.ActiveGeneration(a, thread_id = "t1"):
...
# The registry itself imports nothing beyond the stdlib, so it runs on every
# platform CI offers. The gate lives in routes.inference, which pulls the whole
# inference stack; skip those rather than fail where that stack is not installed.
def _route_gate():
...
def test_tracked_cancel_registers_the_thread_for_its_block():
    # _TrackedCancel is the single place a generation is recorded, so every
    # streaming path picks the registry up without its own bookkeeping.
...
def test_tracked_cancel_shares_its_event_with_the_registry():
    # cancel_all() must reuse the per-run cancel event, not invent a new
    # teardown path: that is what keeps a forced reload from touching
    # llama-server itself.
    _route_gate()  # skips when the inference stack is unavailable
    from routes.inference import _TrackedCancel
...
def _parallel_constants(path: str) -> dict:
    """Read the ``_PARALLEL_*`` module constants out of a file's source.

    Importing run.py would drag in structlog and the rest of the server just to
    read three integers, which is exactly the dependency this file avoids so it
    can run on a bare interpreter on every platform.
    """
    import ast
...
def test_studio_defaults_to_more_than_one_decode_slot():
    # With one slot the admission queue serialises every chat, so parallel
    # conversations would queue instead of streaming together.
...
def test_cli_and_backend_parallel_defaults_agree():
    # run.py's argparse and the typer CLI are separate entry points into the
    # same server; a drift here would make `unsloth studio` behave differently
    # from a direct backend launch.
