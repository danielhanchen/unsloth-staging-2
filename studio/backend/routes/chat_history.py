...
async def replace_thread_messages(
    ...
    if get_chat_thread(thread_id, subject = current_subject) is None:
        raise HTTPException(status_code = 404, detail = f"Thread {thread_id} not found")
    # PR-A2 part 2: reject mismatched body threadId instead of silently
    # rewriting it under the URL thread.
...
async def clear_history(
    ...
    """PR-A3: require ?confirm=true so a misclicked frontend or stray
    third-party request cannot wipe the entire chat history."""
    if not confirm:
        raise HTTPException(
            status_code = 400,
...
async def put_settings(
    ...
    """PR-A4: route-level read-modify-write was racy; the storage helper now
    does the deep-merge inside a single BEGIN IMMEDIATE transaction so two
    concurrent writers cannot drop one another's update."""
    try:
