...
def desktop_runtime_check(
    ...
    """Import the backend for real and report whether it can actually start.

    Complements `verify-install`, which only answers "did the dependency pass
    finish". This one catches a *transitive* dep that no requirements line names
    (e.g. starlette) and any import-time failure, so the two are kept separate
