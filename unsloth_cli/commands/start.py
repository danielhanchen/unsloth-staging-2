...
# Codex's 300000 idle-SSE default trips before a slow local first turn, and each reconnect takes a fresh KV slot.
# A ~7300-token first turn is ~460s of silence; this is 20 minutes.
# Measured 16.1 tok/s on a 2-core box; observed as a request ending at exactly 300056ms.
_CODEX_STREAM_IDLE_TIMEOUT_MS = 1_200_000
_HERMES_ENV_KEY = "UNSLOTH_API_KEY"
_HERMES_PROVIDER = "unsloth"
# Skip the installer's setup wizard: its global API-key prompts block an unattended launch.
# Script and checkout are pinned to one commit.
...
# Hermes refuses to initialize under a 64,000-token window; write_hermes_config scales compaction back to the real one.
# Its error points at model.context_length / auxiliary.compression.
...
# OpenCode selects a model by "<providerID>/<modelID>"; a dedicated id avoids colliding with a user's providers.
...
# Sampling overrides pin a value on the auto-started server; unset means the model's recommended sampling.
...
# One normalized "run tools without prompting" switch: accept every agent's spelling, route it per agent below.
...
# Per-agent flag for "run tools without prompting"; OpenCode (command-scoped) and OpenClaw (config-only) are absent.
_YOLO_COMMAND_FLAGS = {
    "claude": ["--dangerously-skip-permissions"],
    "codex": ["--dangerously-bypass-approvals-and-sandbox"],
    "hermes": ["--yolo"],
    # Pi never prompts per tool call; -a (trust project resources) is its closest equivalent.
    "pi": ["--approve"],
}
...
# Subcommands that reject --auto (OpenCode exposes it on the TUI and `run` only); unknown first positionals are TUI
# paths.
...
def _opencode_supports_native_auto(command: str = "opencode") -> bool:
    ...
    if executable is None:
        # No local binary: assume native --auto is available, since _run installs the current release on launch.
        return True
    try:
...
def _opencode_subcommand(args: list[str]) -> tuple[Optional[str], Optional[int]]:
    ...
    while index < len(args):
        ...
        if any(arg.startswith(f"{option}=") for option in _OPENCODE_GLOBAL_VALUE_OPTIONS):
            index += 1
            continue
        # A non-global option (e.g. --session) is a TUI flag; stop before its value is mistaken for a subcommand.
        if arg.startswith("-"):
            return None, None
        return arg, index
    return None, None
...
def _opencode_native_auto_args(
    ...
    *,
    ...
    # --mini's runMini TUI forces auto=false and never forwards --auto, so fall back to the config permission block.
...
def _hermes_resume_oneshot_args(args: list[str]) -> list[str]:
    ...
    for index, arg in enumerate(rewritten):
        if arg in ("-z", "--oneshot"):
            ...
            # argparse accepts attached short-option values (`-zPROMPT`, `-z=PROMPT`); preserve the value
            # byte-for-byte under -q.
        ...
        # `chat -Q -q` does not set HERMES_YOLO_MODE/HERMES_ACCEPT_HOOKS, so re-add them or a resumed `-z` stalls on
        # an approval. `-z` sets both for a one-shot (hermes_cli/oneshot.py), which has no user at the
        # terminal to answer one. Parity with the flag already typed, not --yolo: a resumed interactive
...
def _subagent_model_id(
    ...
    if variant:
        # A path load is advertised as a bare basename with no ":variant" channel, so a later reload picks for itself.
        typer.echo(
...
def _http_json(
    ...
    try:
        ...
        # A padded /load or /unload commits its 200 early, so a late failure arrives in-band; raise it as the
        # HTTPError below.
...
# A server WE auto-started (never one we merely found). Module scope so failure paths and the atexit backstop reach it.
# Only one agent runs per process.
...
class _DownloadProgressDisplay:
    ...
    def update(self, progress: dict) -> None:
        ...
        if downloaded <= 0:
            return
        # A fully cached snapshot can report 99% with no incomplete bytes; that is not a transfer.
...
class _ModelDownloadProgress:
    ...
    def _configure(self) -> None:
        ...
        if self._disabled:
            return
        # GGUF repos need the selected quant's size (the repo endpoint totals every quant), so resolve the variant
        # first.
...
def _load_model_with_progress(
    ...
    try:
        ...
        if not ok:
            ...
            # A pad-only or half-written body fails json.loads: report the padded 200 that never completed, not a JSON
            # error.
...
def _shutdown_server(server: Optional[subprocess.Popen]) -> None:
    ...
    if server is None or server.poll() is not None:
        return
    if os.name == "nt":
        # terminate()/kill() reach only the parent `unsloth run`; taskkill /T walks the tree so llama-server frees the
        # port and GPU. Matches the taskkill /T /F pattern already used in unsloth/dataprep/synthetic.py.
        try:
            subprocess.run(
...
def _start_studio_server(
    ...
    global _auto_served_server
    # Go through this interpreter, not PATH: PATHEXT resolves `unsloth` to the denied unsloth.exe (issue #8490).
    if sys.platform == "win32":
        # Local import: commands.studio imports at package init after this module, so a top-level import would be
        # circular.
    ...
    # Tools default off = passthrough; --no-cloudflare = loopback only. Healing/nudging travel
    # via the child env so an older re-exec'd run cannot mistake them for llama-server args.
    # Mirrors .github/scripts/serve-unsloth-run.sh.
    command = [
        *launch_head,
        "run",
        "-H",
    ...
    # 0600: this log carries the minted sk-unsloth- key. Unlink first so a stale looser-mode file cannot survive.
    log_path.unlink(missing_ok = True)
    log = os.fdopen(os.open(log_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), "wb")
    # Own session/process group so a mid-session Ctrl+C does not reach the server.
    child_env = os.environ.copy()
    # Use the env equivalent of --reasoning: older managed versions ignore an unknown env var but fail on an unknown
    # flag.
    child_env["LLAMA_ARG_REASONING"] = server.reasoning or "auto"
    # Always written: an inherited value would pin a level the omitted flag leaves alone. 'default' is llama.cpp's
    # sentinel.
    child_env["LLAMA_ARG_REASONING_EFFORT"] = server.reasoning_effort or "default"
    # Marker via env so an older launcher ignores it rather than treating it as a llama-server arg.
    child_env[_START_API_KEY_MARKER_ENV] = "1"
    # Healing/nudging via the env, so they survive a re-exec into an older Unsloth venv.
    # Only write when the operator set the flag; otherwise keep the inherited value, falling back to the start defaults.
    ...
    # Forward sampling pins via UNSLOTH_SAMPLING_*; the backend resolver applies them as a hard override.
    ...
    try:
        while time.monotonic() < deadline:
            ...
            if progress is not None:
                progress.poll()
            # New children emit an early key marker, so wait for the final model banner; older children print the key
            # after load.
...
def _effective_base(base: str) -> str:
    # Normalize UNSLOTH_STUDIO_URL to scheme://host:port: `unsloth run` binds `parsed.port or 8888`. IPv6 stays
    # bracketed.
    parsed = urlparse(base)
...
def _require_studio(
    ...
    *,
    ...
    if base is not None:
        # UNSLOTH_SAMPLING_* pins only reach a server WE launch, so warn rather than silently drop them on the attach
        # path.
        _pinned = [
            _flag
            for _flag, _value in (
    ...
    # Auto-start only for an interactive plain-HTTP loopback target: an https health poll would spin to the startup
    # timeout.
    if (
        serve
        and launch
        ...
        # Normalize to the port `unsloth run` binds, so the health poll and the returned base hit the same server.
        expected = _effective_base(expected)
        load = load or LoadOptions()
        # Leave a bare GGUF repo's variant unset: the server's own quant preference picks and falls back when one is
        # It prefers UD-Q4_K_XL for Unsloth uploads, else Q4_K_M.
        # missing.
...
def _server_buckets(servers: dict, base: str) -> dict:
    # Normalise to {"saved": [...], "minted": [...]}, tolerating a corrupt or legacy bare string/list as minted.
...
def _cached_keys(cache: Path, base: str, source: str) -> list:
    # Keys are scoped per server: "saved" are trusted for that base, "minted" are replayed only after the identity
    # check.
...
def _read_json_object(path: Path) -> Optional[dict]:
    # {} when missing, None when unparseable, so the caller leaves a user-managed file untouched.
    if not path.exists():
        return {}
    try:
...
def _key_accepted(base: str, key: str) -> bool:
    # Only 401/403 means the key is bad; a 5xx or network blip is an outage, so keep the working key rather than
    # minting.
    try:
    ...
    *,
    ...
    if explicit:
        if not auto_started or _key_accepted(base, explicit):
            ...
            return explicit
        # The server was auto-started for this run, so a stray exported UNSLOTH_API_KEY must not fail the launch.

    # Replay a key the user saved for this exact server first, including a tunnelled remote the handshake cannot match.
    for key in _cached_keys(cache, base, "saved"):
        if _key_accepted(base, key):
            _remember_key(cache, base, key, "saved")
            return key

    # find_studio_server() trusts a base after a health check alone, so mint and replay only against a confirmed
    # loopback.
...
def _is_hub_model_id(value: object) -> bool:
    ...
    if len(text) >= 2 and text[1] == ":" and text[0].isalpha():
        return False
    # A hub id is exactly "namespace/name": extra segments must not casefold-match a path on a case-sensitive
    # filesystem.
    # Host independent, unlike the existence probe below.
    parts = text.split("/")
    if len(parts) != 2:
        return False
...
def _model_id_matches(
    ...
    *,
    ...
    if actual == requested:
        return True
    # Case-insensitive matching is only safe against a loopback Unsloth, where _is_hub_model_id's local probe is
    # authoritative.
    # Remotely a two-segment string is indistinguishable from a server-side relative path.
...
def _resolve_model(
    ...
    # Only casefold-match ids against a loopback Unsloth; see the note in _is_hub_model_id.
    allow_casefold = is_loopback_url(base)
    # /v1/models reports the id but not the active variant, so with any explicit load knob defer to /api/inference/load,
    # whose dedup answers already_loaded without reloading.
    ...
    # /v1/models also lists cached-but-unloaded entries, so only attach to one that is actually loaded.
    match = (
        None
    ...
    if requested and match is None:
        load_requested = True
        # A path-loaded GGUF shown as a bare basename can collide with a non-GGUF unsloth/<name>.
        ...
        if preload_check is not None:
            # An explicit knob forces match to None so the server's disk-free dedupe can answer already_loaded.
            # Only the quant is checked below, since any other knob is a real reload.
            ...
            # /v1/models shows a path-loaded GGUF under its basename, so match that spelling too.
            ...
            # /v1/models shows only the basename, so confirm a path request against the loaded identifier, else
            # /new/foo.gguf
            # reads as resident because /old/foo.gguf is.
            ...
            if resident_serves_request and load.gguf_variant:
                ...
                # Casefold, not _normalized_variant (it strips separators): a mistyped Q4KM would skip the gate yet
                # still reload.
        ...
        if active_id and not _model_id_matches(
            ...
            # Same repo id with an explicit quant still replaces the weights, and /v1/models has no variant, so ask
            # status.
            try:
                status = _http_json("GET", f"{base}/api/inference/status", key)
            except Exception:
            ...
            if resident and _normalized_variant(resident) != _normalized_variant(load.gguf_variant):
                ...
                typer.echo("This unloads the current model for every attached session.")
                announced_switch = True
        # Mirror `unsloth run`'s load knobs; keep the default payload as just model_path.
        ...
        try:
            ...
            # If the load was refused before evicting, say so. Not BaseException: Ctrl+C must stay immediate.
        ...
        # Unsloth registers a canonical id that may differ from the path we passed, so match the id the load reports.
    ...
    if match is not None:
        ...
        return match
    if requested:
        # We asked Unsloth to load it and it did not surface in /v1/models; do not hand back an unrelated loaded model.
        _fail(
...
# Mirrors hub.utils.gguf._DRAFTER_KINDS: dflash is also a family name, so only mtp/ and dspark/ are companion folders.
# dflash is the DeepSeek V4 Flash drafter; mirrors is_mtp_drafter_path.
_DRAFTER_KINDS = ("mtp", "dspark", "dflash")
_DRAFTER_DIR_KINDS = ("mtp", "dspark")


def _is_auxiliary_gguf(filename: str) -> bool:
    # Mirrors detect_gguf_model_remote: projectors, separate-file drafters and big-endian builds are not loadable
    # weights.
    # Drafters match by basename prefix or exact parent dir, never substring, since the kind names double as family
    # names.
...
def _direct_gguf_is_companion(path: str) -> bool:
    ...
    if not name.endswith(".gguf"):
        return False
    # Root-independent refusals only: name prefixes read the basename alone, a drafter FOLDER does not.
    if "mmproj" in name:
        return True
...
# Mirrors gguf_variants._DIRECT_SPLIT_RE; five digits exactly, a shorter -001-of-002 name loads as an ordinary file.
...
def _direct_gguf_file_is_ready(path: str) -> bool:
    ...
    try:
        ...
        # A broken symlink is visible here and the .gguf suffix still makes it a load, which fails after teardown.
        ...
        if whole is not False:
            return True
        # Mirror _local_gguf_load_path: an incomplete nominal set still loads when the shard is a symlink to the full
        # set.
        if p.is_symlink():
            target = p.resolve()
...
def _answer_offers_variant(
    ...
    for row in variants:
        if not isinstance(row, dict):
            return True
        # A torn local row proves a load llama cannot serve, so it does not vouch in strict mode; hub partials still
        # count.
        ...
        if isinstance(filename, str):
            # The local resolver also takes the exact shard-stripped stem: full relative spelling only, never a nested
            # basename.
            stem = re.sub(r"-\d{3,}-of-\d{3,}$", "", filename.rsplit(".", 1)[0]).lower()
            if wanted == stem:
                return True
            # Also any whole quant token of the BASENAME (F16-checkpoint-Q4_K_M -> Q4_K_M).
            if any(
                wanted
                in (
...
def _preflight_codex_gguf(
    ...
    *,
    ...
    # Hub-listing preflight for the auto-start path only; with a server running, identifiers resolve against its cwd and
    # cache. Only a complete listing with no .gguf rejects.
    # Mirrors _require_studio's auto-start condition so its "no running server" error comes first.
    ...
    # A bare foo.gguf naming no local file is a shorthand, not a path: the load canonicalizes it.
...
def _attach_gguf_check_for_codex(
    ...
    # Attach path: the server resolves the identifier with its own cwd, cache and token, so ask before the load evicts.
    # A live empty list is definitive; any error defers.
    if not model:
        return
    repo, inline_variant = _split_repo_variant(model)
    # `--model repo:QUANT` is split before the gate runs, so the caller passes the quant on.
    variant = variant or inline_variant
    # Only the owner/name.gguf hub shape gets probed; a .gguf path is GGUF by definition and a bare foo.gguf is
    # shorthand.
    ...
    if (
        ...
        # detect_gguf_model refuses a companion or big-endian build, so the load falls through to transformers after
        # unloading
        # llama-server. Settled by name; the quant never redirects a direct file.
        # `from_identifier` consults it only for a DIRECTORY.
        ...
        # A direct file is loaded as itself, never replaced by a complete sibling matching the quant.
        if not refused and not uncertain:
            # Loopback does not prove a shared filesystem (127.0.0.1 may be an SSH or container forward), so confirm
            # this machine's
            # Unsloth and otherwise defer to the probe.
            if is_loopback_url(base) and verify_studio_identity(base):
                ...
                try:
                    ...
                    is_gguf_dir = False
                if not is_gguf_dir:
                    # On loopback this reads the server's filesystem, so a name absent from a listable directory is
                    # absent for the load
                    # too; an unreadable parent defers.
                    try:
                        probe = Path(os.path.expanduser(repo))
                        missing = (
                            # Only a path this OS spells: a Windows path seen from WSL parses to nonsense, so it
                            # defers to the probe.
                    ...
                    # Only a spelling this OS can judge is settled here: a Windows path read from WSL would vouch for
                    # a file nobody looked
                    # at. An explicit variant also goes to the probe.
                    if not variant and _path_syntax_is_native(repo):
                        return
            # A remote server's filesystem is not ours to read and the load takes the .gguf suffix as authoritative,
            # so ask it.
    # Mirrors the load path's shorthand precedence: the raw name first, then unsloth/<name>. A live answer settles that
    # exact id, so the canonical form must not vouch for an already-answered raw name. Only an error falls through.
    candidates = [repo]
    ...
    for candidate in candidates:
        ...
        if isinstance(variants, list):
            # A cleanable row is an empty leftover <quant>/ folder: it offers a delete, never weights.
            variants = [
                row for row in variants if not (isinstance(row, dict) and row.get("cleanable"))
            ]
        # A raw answer the server calls non-local settles nothing, since the load then canonicalizes to unsloth/<name>.
        # Without the flag (older server) it is still the best evidence there is.
        if (
            "/" not in candidate
            ...
            continue
        # The server answers with the load resolver itself, so it settles the round even on an EMPTY listing.
        if isinstance(variants, list) and isinstance(info, dict):
            offered = info.get("loadable_variants")
            # No allow-list means a direct file, which loads as itself whatever the quant, so the variantless verdict
            # decides.
        ...
        if isinstance(variants, list) and variants:
            # llama.cpp kills the resident model before resolving the quant, so an unservable quant is settled here;
            # local answers
            # take exact labels only.
            # "Local" means resolved_locally or, predating that field, path syntax and bare names, owner/name.gguf
            # exempted.
            hub_shaped = (
                candidate.count("/") == 1
            ...
            # A local answer of only torn rows has no resumable side: llama-server gets the incomplete split only
            # after teardown.
            ...
            # A variantless local load picks from the directory's top level, so rows in quant subdirectories need the
            # variant.
            if (
                local_answer
                and not variant
            ...
            return
        if isinstance(variants, list):
            # Explicit local syntax resolves locally on every server version, so a live empty answer settles the load;
            # deferring
            # would let a GGUF-less directory evict. Marker-less names defer unless the server reports resolved_locally.
            # A bare foo.gguf settles nothing until the canonical form has answered.
...
def _claude_version() -> Optional[tuple]:
    # None = no local `claude`; an unparseable version counts as too old.
    ...
    try:
        ...
        # Pull the X.Y.Z out of the output rather than assuming it is the first token; no match falls through to "too
        # old".
...
def _claude_flags(model_id: str, settings: Optional[str] = None) -> list:
    # KV-cache-preserving flags: claude < 2.1.98 rejects the dynamic-sections flag but already supports --settings.
...
def _claude_local_env(base: str, key: str, entry: dict) -> dict:
    ...
    if window:
        # claude assumes 200k for an unrecognized model id and clamps AUTO_COMPACT_WINDOW; MAX_CONTEXT_TOKENS sets the
        # window.
...
# Copied from openai/codex rust-v0.144.0 models-manager/prompt.md (Apache-2.0), Codex's own unknown-model fallback.
...
def write_codex_config(base: str, model: dict, home: Path) -> None:
    ...
    # oss_provider here too: codex --oss picks the provider from it, and the profile layer must beat config.toml.
    profile_text = (
    ...
    if _codex_supports_model_catalog() and _CODEX_FALLBACK_PROMPT.is_file():
        ...
        # Resolve relative to the profile file; this also survives WSL launching a Windows Codex binary.
...
def write_codex_parent_overlay(overlay: Path) -> Path:
    ...
    if same_source:
        ...
        # A reused overlay must never mix credentials, config or plugins from two Codex homes; legacy overlays have no
        # manifest, so rebuild them once.
        for target in list(overlay.iterdir()):
            _remove_overlay_entry(target)

    # Symlink the user's auth, config, plugins, agents, skills, rules and session state so this is an overlay rather
    # than a
    # stale copy; fall back to directory junctions when Windows denies links, and copy only if neither works.
...
def _opencode_subagent_inline_config(
    ...
    # Keep an inherited inline allowlist usable by the local provider; under V2 global/project rules outrank these.
...
def write_claude_subagent_plugin(path: Path, server_env: dict) -> Path:
    ...
    # This PreToolUse hook replaces Claude's dead-end plan-mode refusal with a reason naming the read-only tool. Skipped
    # under the WSL bridge, where it would run beside the Windows claude.
    # Claude refuses the editing tool in plan mode with readOnlyHint false.
                    ...
                    "PreToolUse": [
                        {
                            ...
                            "hooks": [
                                {
                                    "type": "command",
                                    # runpy, not the path: a missing gate is then an ordinary traceback (exit 1, fails
                                    # open) rather than exit 2, which
                                    # Claude treats as a blocking error.
                                    # The path is base64'd because this string goes through a shell, where $(..), a
                                    # backtick or %VAR% would expand.
                                    "command": (
                                        f'"{sys.executable}" -c '
                                        f'"import base64,runpy; runpy.run_path('
                                        f"base64.b64decode('{_b64_path(gate)}').decode())\""
                                    ),
                                    # A hook with no timeout stalls the parent as long as it hangs; measured unbounded
                                    # past 400s.
                                    "timeout": 10,
                                }
                            ],
...
def _looks_like_path(value: str) -> bool:
    # Only path-valued vars want WSLENV's /p flag; scalar knobs must pass through untranslated.
    return bool(value) and (value.startswith(("/", "\\")) or (len(value) >= 2 and value[1] == ":"))


def _wsl_bridge_names(env: dict, unset_env: tuple) -> tuple:
    # Build the WSLENV share list for a Windows shim reached from WSL; a cleared var carries no value to translate.
...
def _merge_wslenv(current: str, names: tuple) -> str:
    # Index WSLENV entries by bare var name, first-seen order. Our vars apply last so a user's unflagged "HOME" is
    # upgraded
    # to "HOME/p": WSLENV ignores a duplicate name and a bare entry leaves the path untranslated.
    ordered = []
    by_name = {}
...
def _powershell_quote(arg: str) -> str:
    # PowerShell reads single-quoted strings literally; list2cmdline's backslash-escaped quotes are cmd.exe syntax it
    # mis-parses.
...
def _print_env(
    ...
    # The final line is a self-contained one-liner: people copy just it, and a bare `codex`/`claude` would run against
...
def _refresh_windows_path() -> None:
    # Merge Windows registry PATH hives after the process PATH so a freshly installed agent is visible without changing
    # precedence.
    if os.name != "nt":
        return
    try:
...
def _augment_path_with_install_dirs() -> None:
    # Add known install dirs to PATH so a fresh agent resolves without a new shell. A missing home must not drop the
    # managed Node, or a shim fails on `env node`.
    try:
        home = Path.home()
    except (RuntimeError, OSError):
        home = None
    ...
    if current is None:
        # PATH unset: shutil.which() and exec*p* fall back to os.defpath, so keep that. An explicitly empty PATH means
        # "search nothing".
        current = os.defpath
    preferred_node = (
...
def _which_with_install_dirs(name: str) -> Optional[str]:
    # shutil.which(name) plus the known agent install dirs, so a version probe resolves the same binary _launch() will;
    # otherwise an agent only in ~/.local/bin or %APPDATA%\npm is launched with flags an older build
    # rejects. PATH is restored afterward: only _launch() should persist the augmentation.
    original = os.environ.get("PATH")
    _augment_path_with_install_dirs()
    try:
...
def _install_command(install_hint: str) -> tuple[list[str], Optional[dict]]:
    ...
    env = dict(os.environ)
    # dirname, not Path().parent: Path picks its flavour from os.name, which the tests override. Empty means a bare
    # name,
...
def _install_agent(name: str, install_hint: str) -> Optional[str]:
    # Missing agent under --launch: offer its documented install command, then re-resolve on PATH. Consent-based, so
    # no-TTY and declined both return None.
    if not sys.stdin.isatty():
        return None
    typer.echo(f"`{name}` is not installed.")
    # Name the source before the prompt: these are the vendors' own installers, run with the user's privileges, and
    # nothing checks a signature or hash.
    ...
    if result.returncode != 0:
        ...
        _fail(message)
    # Resolve the freshly installed agent without a shell restart: registry PATH plus well-known install dirs.
...
def _wsl_shim_env(
    ...
    if not wsl_env_bridge and not cwd_env:
        return env, ()
    # Bridge PWD via WSLENV (PWD/p) so the Windows shim finds its project root from the live cwd.
    # Do not freeze env["PWD"]: a --no-launch recipe must translate the PWD when run, not when
    # generated.
...
def _resolved_launch_command(
    ...
    if os.name == "nt" and Path(executable).suffix.lower() in {".cmd", ".bat"}:
        # cmd.exe treats CR/LF inside `%*` as command separators and PowerShell rewrites embedded quotes; match complete
        # cmd-shim templates so custom wrappers keep their setup behavior.
...
def _launch(
    ...
    # Resolve well-known install dirs (e.g. ~/.local/bin) first, so an installed agent not yet on PATH is not
    # reinstalled.
    _augment_path_with_install_dirs()
    ...
    if os.name != "nt" and not wsl_env_bridge:
        # Some Node CLIs use PWD, not process.cwd(), for project-root discovery, so do not let a child see a stale PWD.
        child_env["PWD"] = os.getcwd()
    # A no-op handler, not SIG_IGN: exec preserves an ignored signal but resets a caught one, and Ctrl+C must cancel a
    # turn.
...
def _connect(
    ...
    *,
    ...
    # `--model org/name:QUANT` is shorthand for --model org/name --gguf-variant QUANT; split before matching so attach
    # resolves against the loaded `org/name` instead of reloading a `:`-suffixed id and evicting another session.
    if model:
    ...
    try:
        ...
        # A server we just started has the requested model loaded. Only an attach can trigger an evicting load, so
        # _resolve_model runs the pre-load check then.
...
def _run(
    ...
    *,
    ...
    # Some agents (Pi) paint inline and assume a clean screen; click.clear() is a no-op when stdout is not a terminal.
...
def _temporary_agent_config(prefix: str):
    # Reuse the locked session helper: the next launch reclaims homes left by a killed wrapper, and the lock spares
    # live sessions.
    ...
    with contextlib.ExitStack() as stack:
        try:
            ...
            # Attaching to a remote or running Unsloth needs no local auth tree, so fall back to the system temp dir: no
...
def _ephemeral_session_parent(agent: str) -> Optional[Path]:
    ...
    if os.name != "nt" or agent not in _CODEX_SHORT_HOME_AGENTS:
        return None
    # Codex nests a deep curated-plugin checkout below CODEX_HOME and refuses PATH helpers under the system temp dir, so
    # keep the throwaway home short but private to the user.
        ...
        if os.name == "nt":
            ...
            while True:
                try:
                    ...
                    if not blocking:
                        break
                    # LK_LOCK gives up after roughly ten seconds; poll LK_NBLCK so a large stale plugin checkout
                    # cannot fail a launch.
                    time.sleep(0.05)
        else:
            import fcntl
...
def _reclaim_stale_ephemeral_sessions(parent: Path, prefix: str) -> None:
    ...
    for path in parent.glob(f"{prefix}*"):
        ...
        try:
            ...
            continue
        # The wrapper owns the advisory lock, not the Codex child, so give a possibly-live child a full day before
        # treating
        # an unlocked home as stale.
    ...
    try:
        ...
        try:
            with _locked_file(parent / ".cleanup.lock") as cleanup_lock:
                if not cleanup_lock:  # The blocking acquisition should always succeed.
                    raise RuntimeError(f"Could not lock ephemeral session root: {parent}")
                # Release the live marker only after deletion is serialized with startup scavenging, so no scanner
                # races this rmtree.
                active_lock.close()
                if path is not None:
                    shutil.rmtree(path, ignore_errors = True)
...
def _session_config(
    ...
    if launch and not persist:
        ...
        # Never wipe this dir: a previously printed recipe may still be running an agent whose state lives here.
        # Writers merge
        # idempotently and must reset state a previous run's flags left behind (--yolo especially).
...
def write_openclaw_config(
    ...
    # The selected project may already have its own AGENTS.md and git metadata, so do not seed OpenClaw's bootstrap
    # files
    # or initialize a repository.
    defaults["skipBootstrap"] = True
    # OPENCLAW_STATE_DIR does not relocate the workspace; callers pin it to the invocation directory so OpenClaw edits
    # the
    # same project. The managed fallback stays for callers with no explicit workspace.
    if workspace_path is None:
        workspace = path.parent / "workspace"
        workspace.mkdir(parents = True, exist_ok = True, mode = 0o700)
        workspace_path = str(workspace)
    defaults["workspace"] = workspace_path
    # Per-agent paths override agents.defaults.workspace and OPENCLAW_STATE_DIR, so remove stale explicit paths.
    ...
    # Unauthenticated loopback gateway: without auth.mode=none the client will not open the websocket.
    # The daemon still needs `openclaw gateway`.
    gateway = _subdict(config, "gateway")
    gateway.setdefault("mode", "local")
    _subdict(gateway, "auth").setdefault("mode", "none")
    if yolo:
        # OpenClaw gates execution on BOTH tools.exec and a host-local approvals file (the stricter wins), so set both,
        # mirroring `openclaw exec-policy preset yolo`.
        exec_policy = _subdict(_subdict(config, "tools"), "exec")
        exec_policy["host"] = "gateway"
        exec_policy["security"] = "full"
        exec_policy["ask"] = "off"
        # Approvals file in OPENCLAW_STATE_DIR; ask=off means nothing is ever prompted.
        # ask=off makes the runtime socket block unnecessary.
        ...
        # The config dir is reused across runs and OpenClaw reads an omitted exec policy as security=full/ask=off, so a
        # non-yolo run must WRITE a prompting policy rather than delete keys. Only a permissive policy is replaced.
        ...
        # Match ONLY the exact fingerprint --yolo writes (host=gateway, security=full, ask=off, no mode); host=node,
        # host=sandbox and tools.exec.mode are always deliberate user policy.
        # host=auto or an omitted host resolves to security=deny under a sandbox.
        permissive = (
        ...
        # Drop the yolo defaults from the host approvals file; with a prompting tools.exec the stricter layer wins.
        approvals = path.parent / "exec-approvals.json"
        if approvals.exists():
            state = _read_json_object(approvals)
            if state is not None:
                defaults = state.get("defaults")
                # Strip the defaults only on the exact yolo fingerprint; a user-managed policy sharing a field is kept
                # intact.
...
def write_opencode_config(
    ...
    # Keep the provider definition in this private session file; effective filters are adjusted in the inline overlay.
    model_entry = {"name": model["id"]}
    window = model.get("context_length") or model.get("max_context_length")
    if window:
        window = int(window)
        # A custom-provider model with no limit defaults to context 0, silently disabling auto-compaction, so declare
        # the
        # real window and a sane output cap.
    ...
    # Normal mode pins this as the session model; subagent mode leaves the user's models alone and exposes ours to
    # @unsloth.
    ...
    if window and not as_subagent:
        # Compact with ~10% headroom: the fixed 20k-token default buffer over-compacts on a small local context.
    ...
    if yolo:
        # Fallback for commands without native --auto. Rides inline (OPENCODE_CONFIG_CONTENT) so it wins over a
        # project config.
        ...
        # Undo only what --yolo wrote: flip our three explicit "allow" tools back to "ask" and leave any other rule
        # alone.
        # A non-yolo session carries no permission: OPENCODE_CONFIG_CONTENT outranks the project opencode.json we
        # cannot read.
...
def write_hermes_config(base: str, model: dict, path: Path) -> None:
    ...
    if path.exists():
        ...
        if isinstance(loaded, dict):
            ...
            return
    # Hermes reads the key only for a named custom provider (a bare `provider: custom` ignores it), so register it under
    # providers.*.
    ...
    if window:
        window = int(window)
        # Hermes auto-detects context from GET /v1/models, which has no context field, so it can
        # default to 256k and overflow a small model. Pin the real window and compact at 90%.
        # Top-level model.context_length is the window.
        if window >= _HERMES_MIN_CONTEXT:
            ...
            # Below Hermes' 64,000-token floor it refuses to initialize, so claim the floor and shrink the threshold
            # (a fraction of
            # the claimed context_length) to fire at 90% of the REAL window.
            # The auxiliary override keeps the same check from rejecting the compression model.
...
def write_pi_config(base: str, key: str, model: dict, path: Path) -> None:
    ...
    if config is None:
        ...
        return
    before = json.dumps(config, sort_keys = True)
    # Pi reads custom providers from ~/.pi/agent/models.json (HOME-relocated); the key lives in the config, not the env.
    provider_model = {"id": model["id"]}
    window = model.get("context_length") or model.get("max_context_length")
    if window:
        window = int(window)
        # An unspecified model defaults to contextWindow 128000 / maxTokens 16384, so Pi compacts too late and
        # overflows the
        # server; pin the real window and output cap.
...
def claude(
    ...
    if as_subagent:
        ...
        with _session_config("claude-subagent", launch, persist = persist) as config:
            ...
            command = [
                ...
                # Before ctx.args: a forwarded `--` would turn later flags positional. Use the `=` form, since
                # --allowedTools is
                # variadic and a detached value swallows the first forwarded positional.
    ...
    # Claude auto-compacts against its native window; the env above supplies the real one and a 90% threshold. --yolo
    # maps
    # to --dangerously-skip-permissions. IS_SANDBOX stays unset: Claude refuses bypass mode as root without a sandbox.
    # History lives in ~/.claude/projects, so resume with `claude --continue`.
...
def codex(
    ...
    # This preflight runs after _connect may have auto-started a server but before _run takes over its lifecycle, so
    # tear
    # the server down here if it rejects the model.
    try:
...
def openclaw(
    ...
    # Default a bare `unsloth start openclaw` to the local TUI; anything passed through is forwarded verbatim, since a
    # leading "--flag value" is ambiguous and prepending `tui --local` would break the global form.
    ...
    with _session_config("openclaw", launch, persist = persist) as cfg:
        ...
        # Resolve the project only when the recipe executes: a --no-launch command may be generated in one directory
        # and run
        # from another.
...
def opencode(
    ...
    if as_subagent:
        subagent_id = _subagent_model_id(base, key, entry, model, gguf_variant)
        subagent_model = {**entry, "id": subagent_id}
        # Stay append-safe for a bare no-launch recipe: a later `run <prompt>` would make `opencode --auto run ...`
        # parse as
        # the TUI, so keep yolo in the inline fallback.
        route_native_auto = (
        ...
        with _session_config("opencode-subagent", launch, persist = persist) as cfg:
            ...
            # A project opencode.json outranks the session file and could field-merge its own agent.unsloth, so pin
            # ours in the
            # inline overlay.
    ...
    # The inline OPENCODE_CONFIG_CONTENT pins the model in the highest-priority layer, so --model is only added for an
    # interactive bare launch: passthrough can misparse it, and no-launch drivers append a subcommand it would break.
    native_auto = False
    ...
    if ctx.args:
        ...
        # Append-safe base: `opencode --auto run ...` parses as the TUI and the command is unknown here, so keep the
        # fallback.
        opencode_args = _opencode_v2_standalone_args([]) if opencode_v2 else []
        command = [command_name, *opencode_args]
    # opencode keeps sessions in ~/.local/share/opencode (never relocated), so resume already survives exit.
    with _session_config("opencode", launch, persist = persist) as cfg:
        config_path = cfg / "opencode.json"
        # OPENCODE_CONFIG is an overlay between the user's global and project configs, so it adds the provider/model
        # for the
        # session without changing their default. The key lives in the config.
        session_permission = write_opencode_config(
            base,
            key,
            entry,
        ...
        # A project opencode.json outranks OPENCODE_CONFIG, so the session model pin rides OPENCODE_CONFIG_CONTENT
        # while the
        # API key stays in the private file. small_model is pinned too, or a user's small_model on a filtered provider
        # resolves not-found mid-session.
        # V1 filters are ordinary overlays; V2 turns them into policies global/project rules win, so leave those intact.
        inline_config: dict = {
            "model": opencode_model,
...
def hermes(
    ...
    with _session_config("hermes", launch, persist = persist) as home:
        # HERMES_HOME relocates hermes' whole home dir like CODEX_HOME, so the user's ~/.hermes is untouched.
...
def pi(
    ...
    if as_subagent:
        ...
        return
    # Pi defaults to the google provider, so pin our provider/model on the command line; the endpoint itself is only
    # configurable via ~/.pi/agent/models.json.
    command = [
        "pi",
        "--provider",
        _PI_PROVIDER,
    ...
    # --ignore-scripts matches Pi's documented install recipe, so accepting the prompt skips lifecycle scripts.
    with _session_config("pi", launch, persist = persist) as home:
        # Pi prefers PI_CODING_AGENT_DIR over $HOME/.pi/agent, so pin it or an inherited value finds the user's real
        # config.
        ...
        if os.name == "nt" or os.environ.get("WSL_DISTRO_NAME"):
            # Node resolves ~/.pi via USERPROFILE (then HOMEDRIVE + HOMEPATH) on Windows, not HOME, so set them
            # whenever Pi may
            # run as a Windows process. splitdrive yields no drive off a POSIX path, so they stay unset there.
            env["USERPROFILE"] = str(home)
            drive, tail = os.path.splitdrive(str(home))
            if drive:
                env["HOMEDRIVE"], env["HOMEPATH"] = drive, tail
        # Pi paints inline from the cursor with no alternate screen, so give it the clean screen it assumes.
        _run(
            base,
            entry,
            env,
