...
def _patch_trl_rl_trainers_impl(trainer_file = "grpo_trainer"):
    ...
    if "args" in call_args and "model" in call_args:
        mixed_precision = (
            ...
            "elif (not use_bf16 and not use_fp16) and mixed_precision_dtype == 'float32' and float32 and not _bf16_supported():\n"
            "    # A float32 model on a GPU without bf16. The only autocast on offer here is\n"
            "    # float16, whose exponent range is far narrower than the float32 the model was\n"
            "    # deliberately loaded in, so an overflow lands on inf and then NaN. Keeping\n"
            "    # float32 respects both the load dtype and the fact that neither flag was set.\n"
            "    # A bf16 GPU is unaffected: bf16 has float32's exponent range, so autocasting\n"
            "    # there stays safe and keeps the memory saving.\n"
