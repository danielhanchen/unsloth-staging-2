...
def _patch_trl_rl_trainers_impl(trainer_file = "grpo_trainer"):
    ...
    if "args" in call_args and "model" in call_args:
        mixed_precision = (
            ...
            "    # The caller ASKED for float32 at load time and then set neither precision\n"
            "    # flag. The only autocast on offer on a GPU without bf16 is float16, whose\n"
            "    # exponent range is far narrower than the float32 they asked for, so an\n"
            "    # overflow lands on inf and then NaN.\n"
            "    # Gated on the explicit request, because full finetuning upcasts weights to\n"
            "    # float32 by itself, and float16 autocast over float32 master weights is the\n"
            "    # ordinary V100/T4 recipe that must keep working (see #4082).\n"
            "    # A bf16 GPU is unaffected either way: bf16 has float32's exponent range, so\n"
            "    # autocasting there stays safe and keeps the memory saving.\n"
